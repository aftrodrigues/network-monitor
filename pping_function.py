...
def get_data_from_file(file, old_lines = 0):
...
def pping_parser(line):
	"""Faz parser de uma linha do arquivo e retorna uma tupla
...
def make_relatory(file, seconds=float('inf')):
	"""
	...
	while seconds_passed <= seconds:
		...
		print("++++++++++"*7)
		data, lines = get_data_from_file(file, lines)
		if len(data) != 0:
			log.debug('len: [%s] / data: %s' % (lines, data[-1]))
			
			results = processor(results, data)
			#dump_result(results)
...
def set_argparse():
	...
	args_parser.add_argument('-t', '--time', type=int, default=0,
...
def main():
	...
	make_relatory(seconds=args.time, file=args.file)
	clean_up()	
